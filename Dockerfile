...
WORKDIR /src
COPY ["EventAggregator.csproj", "."]
RUN dotnet restore "./EventAggregator.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./EventAggregator.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Этот этап используется для публикации проекта службы, который будет скопирован на последний этап
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./EventAggregator.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
